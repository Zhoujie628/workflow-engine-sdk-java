...
class SpringSpnDemoE2ETest {
    ...
    void defaultLocalDemoNegotiatesOnlyCity1WithoutChangingGlobalSwitch() throws Exception {
        ...
        try {
            ...
            try (var input = java.nio.file.Files.newInputStream(log)) {
                ...
                org.junit.jupiter.api.Assertions.assertTrue(evidence.contains("NEGOTIATION_DEMO enabled=true, city=city1"));
                org.junit.jupiter.api.Assertions.assertTrue(evidence.contains("[SpnDomainAgentCity1Executor] NEGOTIATION_APPLIED"));
        ...
        try {
            ...
            try (var input = java.nio.file.Files.newInputStream(log)) {
                ...
                for (String city : java.util.List.of("City1", "City2")) {
                    org.junit.jupiter.api.Assertions.assertTrue(evidence.contains(
                            "[SpnDomainAgent" + city + "Executor] NEGOTIATION_APPLIED"), city);
                }
                org.junit.jupiter.api.Assertions.assertTrue(evidence.contains("port=P781-珠江新城-PTN7900-23-TPA1EG24-17"));
                org.junit.jupiter.api.Assertions.assertTrue(evidence.contains("port=P882-珠江新城-PTN7900-23-TPA1EG24-11"));
            }
        } finally {
    ...
    void missingPortNegotiatesAndCompletesThroughTheSameSpringWorkflow() throws Exception {
        ...
        try {
            ...
            try (var input = java.nio.file.Files.newInputStream(log)) {
                ...
                org.junit.jupiter.api.Assertions.assertTrue(evidence.matches("(?s).*\\\"performative\\\"\\s*:\\s*\\\"PROPOSE\\\".*"));
                org.junit.jupiter.api.Assertions.assertTrue(evidence.matches("(?s).*\\\"performative\\\"\\s*:\\s*\\\"ACCEPT\\\".*"));
    ...
    protected String negotiationWireBoundary() { return "DIRECT_HTTP"; }
    ...
    private void assertNegotiationWireEvidence(String evidence) {
        ...
        org.junit.jupiter.api.Assertions.assertTrue(records.stream().anyMatch(record ->
                record.startsWith("[" + negotiationWireBoundary() + "] REQUEST")
                        && record.contains("requestId=" + requestId)
                        && record.contains("A2A-Extensions: " + uri)), "Missing actual negotiation activation header");
        ...
        org.junit.jupiter.api.Assertions.assertTrue(records.stream().anyMatch(record ->
                record.contains("RESPONSE_BODY") && record.contains("=== SSE data (JSON display; not wire text) ===")
                        && record.contains("\"performative\": \"PROPOSE\"")), "Missing observed PROPOSE SSE data");
    }
    ...
    void northboundComplaintCompletesAndResourcesClose() throws Exception {
        ...
        try {
            ...
            try (var input = java.nio.file.Files.newInputStream(log)) {
                input.skipNBytes(offset);
                String evidence = new String(input.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
                org.junit.jupiter.api.Assertions.assertTrue(evidence.contains("NEGOTIATION_DEMO enabled=false"));
