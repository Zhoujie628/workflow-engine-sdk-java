...
/**
 * Mock {@link OrderGatewayClientRuntime.OrderSession} that bypasses the Eastcom SDK entirely and
 * does plain HTTP forwarding to the mock gateway server.
 *
 * <p>Previously this class extended {@code OrderHttpSessionClient} and overrode {@code
 * login/init/execute}. After migrating to the officially documented {@code HttpClient} API, the
 * production adapter no longer uses {@code OrderHttpSessionClient}, so this mock now implements
 * {@link OrderGatewayClientRuntime.OrderSession} directly.
 *
 * <h2>Flow</h2>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The {@code targetUrl} (set via constructor) stores the target OMC URL. The {@code execute()}
 * call forwards the HTTP request to the mock gateway with an {@code X-Target-URL} header, and the
 * gateway pipes it to the OMC.
 */
public class MockOrderHttpSessionClient implements OrderGatewayClientRuntime.OrderSession {
    ...
    try {
      ...
      if (headers != null) {
        for (Map.Entry<String, String> entry : headers.entrySet()) {
          ...
          if (lowerKey.equals("content-type") || lowerKey.equals("x-target-url")) continue;
  ...
  public void executeStreaming(
    ...
    try {
      ...
      log.info(
          "[MockOrderClient] STREAM_DONE chunks={}, elapsedMs={}", chunks, elapsedMillis(started));
    } catch (Exception e) {
  ...
  private HttpURLConnection prepareConnection(OrderHttpSessionStrRequest request, int timeoutMillis)
      throws IOException {
    String uriPath = request.getUriPath();
    HttpURLConnection conn = openConnection(gatewayUrl + (uriPath != null ? uriPath : ""));
    String method = request.getMethod();
    conn.setRequestMethod(method != null && !method.isEmpty() ? method : "POST");
    conn.setDoOutput(true);
    conn.setConnectTimeout(30_000);
