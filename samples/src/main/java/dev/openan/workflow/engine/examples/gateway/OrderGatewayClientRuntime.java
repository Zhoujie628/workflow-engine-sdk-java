 ...
 * use the JAR's undocumented static login helper, so this logical lifecycle must not be interpreted
 * as an {@code OrdersClientImpl} login/logout session. Agent-to-NE routing is supplied by {@link
...
public final class OrderGatewayClientRuntime
  ...
  private static void logSseFrame(String requestId, String agentName, int frame, String sseFrame) {
    if (!protocolLog.isDebugEnabled()
        || !booleanSetting("WORKFLOW_ENGINE_PROTOCOL_MODEL_PREVIEW", false)) {
      return;
    }
    protocolLog.debug(
  ...
  /**
   * Pretty-prints an SSE frame for protocol logging: control fields ({@code id:}, {@code event:},
   * ...) stay as-is, and the {@code data:} payload is indented as JSON. A frame without {@code
   * data:} lines is treated as one plain JSON payload.
   */
  ...
  private static void logProtocolRequest(
      String requestId, String agentName, String ne, OrderHttpSessionStrRequest request) {
    if (!protocolLog.isDebugEnabled()
        || !booleanSetting("WORKFLOW_ENGINE_PROTOCOL_MODEL_PREVIEW", false)) {
      return;
    }
  ...
  private static void logProtocolResponse(
      String requestId, String agentName, int frame, OrderResponse response) {
    if (!protocolLog.isDebugEnabled()
        || !booleanSetting("WORKFLOW_ENGINE_PROTOCOL_MODEL_PREVIEW", false)) {
      return;
    }
    ...
    headers.forEach(
        (name, value) ->
            formatted
                .append(name)
                .append(": ")
                .append(!includeSensitive && isSensitiveHeader(name) ? "***REDACTED***" : value)
                .append('\n'));
  ...
  public org.a2aproject.sdk.spec.Task getTask(
    ...
    return executeTaskRequest(
        agentCard, route, taskId, "GET", route.taskPath(null, taskId, ""), "", callContext);
  }
    ...
    if (!supportsStreaming(agentCard)) {
      return CompletableFuture.failedFuture(
          new IllegalStateException(
              "subscribeToTask requires agent streaming capability: " + agentCard.name()));
    }
  ...
  private Task executeTaskRequest(
    ...
    try {
      ...
      logProtocolResponse(requestId, agentCard.name(), 1, response);
      validateResponse(response);
      org.a2aproject.sdk.grpc.Task.Builder task = org.a2aproject.sdk.grpc.Task.newBuilder();
      ...
      if (handle != null) handle.invalidate();
      throw new IllegalStateException(
          "Eastcom gateway task " + method + " failed for " + taskId + ": " + e.getMessage(), e);
    }
  }
  ...
  private SendMessageResult subscribeToTaskBlocking(
    ...
    try {
      ...
      if (handle != null) handle.invalidate();
      throw new IllegalStateException(
          "Eastcom gateway subscribeToTask failed for " + taskId + ": " + e.getMessage(), e);
    }
  }
  ...
  private List<ClientEvent> executeBlocking(
    ...
    validateResponse(response);
    List<ClientEvent> events = responseParser.parseNonStreaming(response.body(), eventSink);
    if (events.isEmpty()) {
  ...
  private static final class ConversationSessionManager implements AutoCloseable {
    ...
    private ConversationSessionHandle acquire(
      ...
      ConversationSessionKey key = new ConversationSessionKey(contextId, route.ne(), channel);
      while (true) {
        ensureOpen();
    ...
    private void release(ConversationSessionKey key, ManagedSession entry, boolean invalidate) {
      try {
  ...
  private record DefaultOrderSessionFactory(OrderConfig config) implements OrderSessionFactory {
    ...
    public OrderSession open(AgentGatewayRoute route) {
      ...
      try {
        ...
        log.info(
            "[OrderGateway] SESSION_READY ne={}, elapsedMs={}", route.ne(), elapsedMillis(started));
        return adapter;
      } catch (RuntimeException e) {
        log.error(
