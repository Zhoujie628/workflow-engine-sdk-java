...
public class WorkbenchControlPoint extends DefaultControlPoint {
    ...
    public WorkbenchControlPoint(String a2atEnvPath, NegotiationStrategy negotiationStrategy,
            boolean demoNegotiationEnabled) {
        ...
        this.contentClient = dev.openan.workflow.engine.examples.util.A2ATInitialization.create(
    ...
    public CompletableFuture<MessageContent> onTask(
            TaskRequest request) {
        Map<String, Object> data = dev.openan.workflow.engine.examples.demo.SpnCasePrompts.complaintData(request);
        return CompletableFuture.supplyAsync(() -> dev.openan.workflow.engine.examples.util.BusinessSdkErrors.call("task-generation", () -> {
            var generated = contentClient.generateTaskPromptFromDataWithSchema(
                    data, dev.openan.workflow.engine.examples.demo.SpnCasePrompts.privateLineComplaintSchema(),
                    net.openan.a2at.sdk.core.model.StandardTemplates.PRIVATE_LINE_COMPLAINT_URI);
            MessageContent content = dev.openan.workflow.engine.client.A2atMessages.from(generated,
                    List.of(new org.a2aproject.sdk.spec.TextPart(request.getInstruction())));
            if (dev.openan.workflow.engine.examples.demo.SpnCasePrompts.injectNegotiation(request.getStepName(), demoNegotiationEnabled)) {
                // Explicit local-demo fault injection, not a production generation fallback.
                Map<String, Object> metadata = new java.util.LinkedHashMap<>(content.metadata());
                metadata.put(dev.openan.workflow.engine.client.A2ATExtension.TASK_T.uri(),
                        dev.openan.workflow.engine.examples.demo.SpnCasePrompts.withoutTaskObject(
                                generated.buildMetadataContent().get(dev.openan.workflow.engine.client.A2ATExtension.TASK_T.uri()).toString()));
                log.info("[onTask] DEMO_NEGOTIATION agent={}, fault=missing-port, source=explicit-sample-switch",
