...
/** Loads credential profiles used only by the sample Eastcom authentication adapter. */
final class EastcomCredentialConfigLoader {
  ...
  private EastcomCredentialConfigLoader() {}

  static Map<String, Map<String, Map<String, Object>>> load(String path) {
    if (path == null || path.isBlank()) {
      throw new IllegalArgumentException("Eastcom OMC credentials path must not be blank");
    }
    try {
      ...
      throw new IllegalStateException(
          "Failed to load Eastcom OMC credentials from " + path + ": " + e.getMessage(), e);
    }
  }
  ...
  private static Map<String, Object> readRoot(String path) throws Exception {
    if (path.startsWith("classpath:")) {
      ...
      try (InputStream input =
          EastcomCredentialConfigLoader.class.getClassLoader().getResourceAsStream(resource)) {
        if (input == null) {
          throw new IllegalStateException(
              "Eastcom OMC credentials classpath resource not found: " + resource);
        }
    ...
    if (!file.isFile()) {
      throw new IllegalStateException("Eastcom OMC credentials file not found: " + path);
    }
    ...
    for (Map.Entry<String, Object> entry : agents.entrySet()) {
      ...
      if (profile == null) {
        throw new IllegalArgumentException(
            "Unknown Eastcom credential profile '" + profileName + "' for agent " + entry.getKey());
      }
