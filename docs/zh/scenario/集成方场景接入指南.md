...
> 交付基线：workflow engine `0.0.6`，A2A-T SDK
> `1.1.0`（commit `e42c83acce3e5ac2c245d36546ced0fa017b2b58`），
> A2A Java SDK `1.2.0.Final`，东信 Order SDK `1.1.19`。本执行引擎是首发版本，
...
| 交付物                                                       | 用途                                                                   | 责任方                                  |
|--------------------------------------------------------------|------------------------------------------------------------------------|-----------------------------------------|
| `workflow-engine:0.0.6`、`spring-boot-a2a-starter:0.0.6`         | 通用工作流协议与 Spring A2A 服务端                                     | 执行引擎方                              |
...
| `WorkflowEngineClient`  | 否            | 是              | 工作流与独立任务门面 (Task-T + 协商循环)                 |
...
Spring Boot starter 默认不启动 A2A 服务端。集成方必须显式设置 `a2at.server.enabled=true`，starter 才会注册
`AgentCard`、`RequestHandler`、`RestHandler`、`A2AController` 等 Bean；同时需提供一个标注 `@Component` 的 `AgentExecutor` 实现。
  ...
  "profiles": {
    "spn-omc-default": {
      "bearerAuth": {
        ...
        "order_token_header": "accessSession",
        "token_ttl": 3600
      }
    }
  },
...
| `order_token_header` | String | `accessSession`    | Order 模式优先读取的响应 Header；缺失时回退 `token_field`         |
...
（例如 `accessSession`），不存在时读取 `token_field`
...
编排中心 HTTPS 的 LoadPsop.search/load 在显式 `sslVerify=false` 时，按连接跳过证书链和主机名校验，
因此受控联调可直接设置 `A2A_SSL_VERIFY=false`，不用在执行引擎端下载或配置编排中心证书。
服务端仍需要 HTTPS 证书；如服务端要求 mTLS，不能用此开关替代客户端证书。
该行为不修改 JVM 全局设置，不改变南向直连或东信 SDK 的 TLS 策略。生产保持 `true`，
使用正确 SAN 和 JVM 信任库；引擎南向的 `caCertsPath` 不会自动应用于 LoadPsop。
...
分开。耗尽／超时不自动生成 Abort；若已知远端任务仍为非终态，引擎会尽力调用 A2A CancelTask 清理后再报告本地失败。 结束回复保留收到的
id/round/maxRounds，最后一轮仍可回答。
...
- WorkflowEngineClient.sendTask (String, MessageContent)：DAG 外执行最终内容，复用任务等待与协商闭环； DAG 内同样由执行器调用
  sendTask，但会把工作流任务上下文保留给协商回调。
