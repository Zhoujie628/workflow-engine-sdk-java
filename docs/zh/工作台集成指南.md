...
> 面向传输工作台研发团队。本文档以 SPN 专线故障跨城协同诊断联创场景为例，从工作台视角说明接入Workflow执行引擎 SDK 的完整路径。
...
序号 1、2 在工作流执行过程中发生，由 SDK 的 `WorkflowEngineClient` 承载。序号 3、4 是前置动作，在工作流启动前完成，由 SDK 的 `ExtensionSender` 承载。两条路径使用同一类底层传输组件 `A2ATransport`，但生命周期不同：工作流发送使用任务级 transport，Notification-T 订阅使用工作台级 transport，避免工作流结束时关闭长连接。
...
public class WorkbenchOrchestrator {
    ...
    public String run(String messageText) throws Exception {
        ...
        // 3. 前置预定位使用工作台级 transport
        // 生产环境应在服务启动时创建并持有
        A2ATransport extensionTransport = new A2ATransport(
                agentCards,
                null,
                WorkflowEngineClientConfig.builder()
                        .sslVerify(sslVerify)
                        ...
                        // })
                        .build());
        ExtensionSender extensionSender = new DefaultExtensionSender(extensionTransport);

        // 4. 前置预定位: 向每个 OMC 下发授权策略 + 订阅通知
        prePositionExtensions(extensionSender, agentCards);

        // 5. 工作流发送使用任务级 transport
        A2ATransport taskTransport = new A2ATransport(
                agentCards,
                null,
                WorkflowEngineClientConfig.builder()
...
这两个方法返回 `CompletableFuture<SendMessageResult>`，`.join()` 阻塞等待首个事件 (订阅确认) 完成。SDK 内部自动处理 Authorization-T / Notification-T 报文构造和认证头注入（来自 `AuthProvider` 或 credentials 配置）。对于 `sendNotification`，`.join()` 返回后 SSE 长连接仍然存活，后续事件持续推送到回调，直到工作台主动关闭 transport。
...
两个门面分别使用独立的 `A2ATransport` 生命周期。`WorkflowEngineClient` 使用任务级 transport，工作流结束后由 `ExecutePsop` 关闭；`ExtensionSender` 使用工作台级 transport，承载 Notification-T 长连接，直到服务关闭。
...
sequenceDiagram
    ...
    Note over WB: 3. 创建工作台级 A2ATransport + ExtensionSender
    Note over WB: 4. 创建任务级 A2ATransport + WorkflowEngineClient
    end
