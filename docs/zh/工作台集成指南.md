...
private void prePositionExtensions(ExtensionSender sender, List<AgentCard> agentCards) {
    for (AgentCard card : agentCards) {
        ...
        try {
            ...
            recordAuthorizationFailure(name, e); // 只影响本次授权，不阻止订阅或工作流
        }
...
授权和订阅必须分别捕获、分别记录结果：授权失败仍要尝试订阅，订阅失败不回滚授权；两者任何一种结果都不能作为工作流启动条件。已经打开的订阅应在等待 ACK 前登记到工作台生命周期，确保后续步骤异常时仍能精确关闭资源。
