...
`samples` 是可运行参考实现，不是工作台生产依赖。通用能力通过前两个 Maven artifact 接入；东信专用代码只存在 `dev`，应被迁入工作台自己的
adapter 模块，不能复制进通用
...
使用任务级实例，Authorization-T 一次性实例在 ACK 后释放， Notification-T 使用工作台级长连接实例。
...
`workflow-engine` 仅传递依赖 `a2a-t-core:1.1.0`，不引入内容生成客户端。 工作台若调用生成／校验 API，需显式依赖
...
生产项目不建议直接依赖 `samples` 模块。应将上述模式复制到工作台工程，或抽成工作台自己的东信适配模块；
...
`sslVerify(false)` 只作用于引擎自身创建的 HTTP/JSON-RPC transport；工作台通过东信 SDK 获取 Token 或转发消息时的 TLS 策略，由该
SDK 客户端自行配置。
