...
graph TD
    ...
    OMC1["SPN Domain Agent<br/>(城市1 OMC)"]
    OMC2["SPN Domain Agent<br/>(城市2 OMC)"]

    UPPER -->|"A2A-T Task-T 下发原始诊断任务"| WB
    WB -->|"A2A-T 下发诊断"| OMC1
    WB -->|"A2A-T 下发诊断"| OMC2
```

工作台是编排方。它接收 上层故障系统通过 A2A-T 下发的原始诊断任务，编排下游多地市 SPN Domain Agent (OMC) 执行故障根因诊断，汇总结果并上报。
...
##### 通过东信 SDK 获取 OMC Token

该场景的责任划分是:

- `AuthProvider`: 获取并缓存 OMC Token，把认证 Header 写入 `headers`
- `A2AJavaClientRuntime`: 负责 A2A 消息到东信 SDK 请求的协议转发，不关心 Token 如何获取
...
- `OrderSessionFactory` / `ConversationSessionManager` 管理东信会话
...
生产项目不建议直接依赖 `samples` 模块。应将上述模式复制到工作台工程，或抽成工作台自己的东信适配模块；`OrderGatewayClientRuntimeTest`、`GatewayA2AResponseParserTest` 和 `ClientRuntimeFactory` 可作为迁移时的参考测试与配置结构。
    ...
    private static String buildTargetedTaskMessage(String step) {
        return switch (step) {
            case "diagnosis_city1" -> """
                ...
                请针对城市1 OMC 侧端口故障进行诊断。""";
            case "diagnosis_city2" -> """
                ## 任务
                SPN 专线故障诊断 - 城市2 OMC 侧
...
public CompletableFuture<String> onNegotiation(
    ...
    String clarification = "城市1-城市2间 SPN 专线中断，"
            + "城市1 OMC 告警端口 Down，光功率 -28dBm。"
            + "请根据此上下文修正参数后重新执行诊断。";
...
- 工作台在 `onTask` 中调用 `engineClient.sendMessage()`，SDK 自动处理 Task-T 提示词生成 + 认证头注入 + SSE 流式发送 + Negotiation-T 协商循环
...
4. **确认会话复用**: `OrderGatewayClientRuntime` 日志中 `SESSION_REUSE` 而非每次 `SESSION_OPEN` → 同一 NE 应复用会话，避免重复登录
5. **确认 Connection 单例**: `ClientRuntimeFactory.create()` 在 ORDER 模式下返回单例 `sharedOrderRuntime`，而非每次 new
...
| `ExtensionSender` | `sendExtensionMessageFromData` | `CompletableFuture<SendMessageResult> sendExtensionMessageFromData(String, String, Map, Map, A2ATExtension)` | 结构化 Authorization-T 一次性操作（推荐） |
| `ExtensionSender` | `openNotificationFromData` | `CompletableFuture<NotificationSubscription> openNotificationFromData(String, String, Map, Map, Consumer)` | 结构化 Notification-T 长连接订阅（推荐） |
| `ExtensionSender` | `sendAuthorization` / `sendNotification` | 自然语言输入重载 | 兼容入口；生产有结构化业务数据时不优先使用 |
