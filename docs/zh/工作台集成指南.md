...
A2A-T SDK 的全部调用如下。授权与订阅是独立于工作流 DAG 的**预定位操作**：工作台在任务下发前
生成授权与订阅报文，随 Task-T 同发（或经独立通道在任意业务时机发送），OMC 在接收时先行校验执行，
先于诊断与协商发生。样例覆盖全部 8 个 validate 接口和全部 7 个 fromData 生成接口，均为真实调用；
fromText 生成系列未在样例中使用（见文末说明）。
...
| 2 | 工作台生成抢通白名单授权请求（预定位，随任务同发；也可经独立通道在任意业务时机发送） | 工作台（客户端） | `A2ATClient.generateAuthPromptFromDataWithSchema` | `ExtensionPrePositioner` |
| 3 | 工作台生成业务抢通事件订阅（预定位；也可经独立长连接通道订阅） | 工作台（客户端） | `A2ATClient.generateNotificationPromptFromDataWithSchema` | `ExtensionPrePositioner` |
| 4 | 工作台按地市分别生成 Task-T，携带预定位扩展并行下发两地市 OMC | 工作台（客户端） | `A2ATClient.generateTaskPromptFromDataWithSchema` | `SpringSpnDemo` |
| 5 | OMC 校验 Task-T，提取任务对象/上下文；参数完整则直接诊断（City2） | OMC（服务端） | `A2ATServer.validateTaskPromptAndDataFilling` | `NegotiationBaseAgentExecutor` |
| 6 | OMC 校验预定位授权报文并按白名单执行，返回 ACK | OMC（服务端） | `A2ATServer.validateAuthPromptAndDataFilling` | `PrePositionedExtensionHandler` |
| 7 | OMC 校验预定位订阅报文，建立 SSE 上报；抢通结果经长连接推回工作台 | OMC（服务端） | `A2ATServer.validateNotificationPromptAndDataFilling` | `PrePositionedExtensionHandler` |
