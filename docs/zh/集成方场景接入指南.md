...
| 3    | Authorization-T | 抢通授权          | 集成方 -> OMC                                   | 工作流启动前，前置预定位     |
| 4    | Notification-T  | 抢通结果订阅/上报 | 集成方 -> OMC (订阅) / OMC -> 集成方 (SSE 上报) | 工作流启动前订阅，执行中上报 |
...
`A2ATransport`，但 runtime、context 和生命周期互相隔离：工作流发送使用任务级 transport，Notification-T 订阅使用集成方级
...
- 如果集成方通过 `AuthProvider` 自行获取 OMC Token，推荐配置 `"securityRequirements": []`，`securitySchemes` 可以保留为能力声明
...
| 自定义 AuthProvider | 集成方自管 Token、企业 SSO、非标准认证头，或经自定义网关/协议转发获取 OMC Token | 实现 `AuthProvider` 接口   |
...
集成方业务代码通过此配置初始化 A2A-T SDK，生成和校验协议内容。执行引擎不读取 LLM 配置，也不生成提示词。示例在配置缺失或生成失败时明确报错，不把原始输入降级为 Task-T 正文。
...
workflow、cards、callbacks、intent 由宿主取得或构造。 runtime 为 null 时使用默认直连实现；自定义 runtime 只替换传输，不替换业务回调。
...
`openNotification` 建立的 SSE 订阅通道是长连接。SDK 的 `A2ATransport` 管理连接池和超时。集成方应在服务关闭时调用
`extensionTransport.close()` 释放连接。`ExecutePsop` 仅自动关闭它自行创建的 client/transport；传入已有 engineClient 时由集成方负责关闭。不要把任务级
...
- 前置预定位 transport 是否为集成方级生命周期，工作流 transport 是否为任务级生命周期
- onTask 是否仅准备 MessageContent，由引擎判断远端终态；本地 onSelfTask 是否明确返回业务成功或失败
- `openNotification` 是否传入回调，并在服务关闭时释放前置预定位 transport
