...
- `WorkbenchExtensionLifecycle`：拥有工作台级流程外操作。Authorization-T 每个目标 Agent 执行一次，收到响应后立即释放请求会话；Notification-T 使用独立 transport 持续保持，在收到预期 `recovery-result`、显式取消订阅或工作台进程关闭时释放。
...
# 经指令平台调用 OMC 登录接口，读取响应头 bearToken
...
mvn -B -f samples/pom.xml spring-boot:run `
  "-Dspring-boot.run.main-class=dev.openan.workflow.engine.examples.demo.SpringSpnDemo" `
  "-Dspring-boot.run.arguments=--a2a.transport-mode=order"
```
...
2. OMC Bearer Token 由 `EastcomTokenService` 通过同一个东信 `HttpClient` 调用 OMC 登录接口获取。请求体使用 `${ne:username}`、`${ne:password}`，由指令平台用目标 NE 的协议参数替换，工作台不保存 OMC 密码。

OMC 登录响应的 Token 位于响应头 `bearToken`。`EastcomAuthProvider.applyAuth(...)` 在每次 A2A-T 发送前按 Agent 获取或刷新 Token，并写入 `Authorization: Bearer <token>`。如果 OMC 已返回带 `Bearer` 前缀的值，不会重复拼接前缀；如果已有同名 Header 且值冲突，则拒绝发送，避免静默覆盖认证信息。

`ClientRuntimeFactory` 在 `order` 且 `omc-auth-enabled=true` 时创建一个线程安全的 `EastcomTokenService`/`EastcomAuthProvider`。同一个 Provider 被 Task-T、Authorization-T、Notification-T 三套 transport 配置引用，以共享按 Agent 隔离的 Token 缓存；三套 transport/runtime/协议通道本身仍完全独立。Order 模式不再读取 `credentials.json` 获取 OMC Token；直连模式继续支持原来的 credentials 认证，且不会创建东信 Provider。
