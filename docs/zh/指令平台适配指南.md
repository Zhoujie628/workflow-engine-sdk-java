...
1. 按 [A2A-T SDK 依赖说明](A2AT-SDK-DEPENDENCY.md)使用 Maven Central 的 `1.1.0` 制品，并从企业制品库或供应商取得
...
指令平台只负责通道建立和透明转发。Task-T、Negotiation-T、Authorization-T、Notification-T 扩展仍由执行引擎和 OMC 在协议端点处理，扩展
metadata 和请求头会随 A2A 请求一起发送。SPN 主工作流只使用 Task-T；Negotiation-T 保留为独立协议用例，不通过已废弃的
`startNegotiation` 等接口进入主流程。
...
Order 转发同时检查 SSE data 内的顶层 `status/title/detail` 错误对象。 即使 HTTP 响应头为 200，收到 400/429 等问题对象也立即失败，保留
OMC 的业务原因， 不把它当成 A2A 任务事件、协商 Propose 或等待后续任务的信号，也不会自动重发请求。
详见[远端错误响应](INTEGRATION_GUIDE.md#远端错误响应)。

失败后应区分“调用方已收到错误”和“平台到 OMC 的连接已释放”。当前 SDK 1.1.18 的
`ReactorNettyBridgeHandler.channelInactive` 在本地连接关闭时完成请求流； 模拟平台据此停止本次转发，记录
`FORWARD_CANCEL_REQUESTED` / `FORWARD_CANCELLED`， 不关闭其他 NE 请求或独立 Notification-T 订阅，也不把 problem 对象改写成成功终态。
...
`SESSION_CREATE/SESSION_READY/SESSION_SETUP_FAILED` 日志区分客户端创建和业务阶段。Token 只记录获取/刷新事件，不打印 Header
值。
