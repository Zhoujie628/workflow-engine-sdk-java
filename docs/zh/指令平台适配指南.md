...
> `1.0.0-0de5a27`、A2A Java SDK `1.2.0.Final`、东信 `order-shaded-client:1.1.18`。本文只描述当前版本，
...
平台密码和 client secret 不会进入协议日志，Authorization、Cookie、API Key、Token、Secret
等 Header 默认脱敏。要在 IDEA/受控联调环境查看 `PROTOCOL - >>>/<<<` 和完整 Body：

```text
logger.protocol.name=PROTOCOL
logger.protocol.level=DEBUG
-DWORKFLOW_ENGINE_PROTOCOL_INCLUDE_BODY=true
```

`WORKFLOW_ENGINE_PROTOCOL_INCLUDE_SENSITIVE_HEADERS` 保持 `false`；只有经批准的临时排障才允许
设为 `true`。如果控制台没有协议日志，先确认 IDEA 运行 classpath 使用
`samples/src/main/resources/log4j2.properties`，再检查 `logs/spn-demo.log`。
