...
1. 按 [A2A-T SDK 依赖说明](A2AT-SDK-DEPENDENCY.md)安装固定 commit 的 Maven 制品，并从 企业制品库或供应商取得
...
4. .sendSse (SseListener)：SSE 流式调用。SseListener.onHeader (HttpClientResponse) 收到首帧 HTTP 状态码和响应头；onBodyString
   (String) 收到每个 SSE 事件体。sendSse 阻塞直到平台自然结束 SSE 流。
...
OMC 登录响应优先从 Profile 的 `order_token_header`（默认 `bearToken`）提取；Header 不存在时 从 `token_field`（默认
