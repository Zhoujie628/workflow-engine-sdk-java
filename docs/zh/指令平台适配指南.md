...
graph TD
    ...
    OHSC["HttpClient<br/>create/send/sendSse"]
...
按照东信《联机指令平台接口说明 v1.8》(2026-01-27) 的 SDK 接入说明，执行引擎使用官方承诺的 HttpClient API，不再依赖内部 RSocket-RPC OrderHttpSessionClient：
...
5. .secure(spec -> spec.sslContext(SslContextBuilder.forClient().trustManager(InsecureTrustManagerFactory.INSTANCE).build()))：HTTPS 设备跳过证书校验（erifyssl=false）。HTTP 设备不需要设置。
...
4. 根据 AgentCard.capabilities.streaming 选择发送方式：
   - alse：调用 /message:send，通过 httpClient.post().uri().header().body().send() 获取单个 SendMessageResponse。
   - 	rue：调用 /message:stream，通过 httpClient.post().uri().header().body().sendSse(listener) 逐帧消费 SSE 事件。
...
登录超时与业务请求超时：SDK 内部管理平台连接，EASTCOM_ORDER_LOGIN_TIMEOUT_SECONDS 仍用于配置 ServerInfo；EASTCOM_ORDER_TIMEOUT_SECONDS 控制 A2A 请求的 
esponseTimeout。可通过 SESSION_CREATE/SESSION_READY/SESSION_SETUP_FAILED 日志区分建链和业务阶段。
