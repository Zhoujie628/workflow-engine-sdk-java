...
> 适用基线：`dev` 分支，workflow engine `0.0.8`、A2A-T SDK
> `1.1.0`、A2A Java SDK `1.2.0.Final`、东信 `order-shaded-client:1.1.19`。本文只描述当前版本，
> 不包含旧 A2A-T SDK 或旧 Order 适配接口。

## 集成方拿到代码后的最短路径

1. 按 [开发者指南](DEVELOPER_GUIDE.md)使用 Maven Central 的 `1.1.0` 制品，并从企业制品库或供应商取得
   `order-shaded-client:1.1.19`。
...
`INPUT_REQUIRED` 响应时，引擎在同一逻辑任务中进入协商续发。引擎同样支持 A2A-T 先协商后建任务的无任务载体：OMC 在创建任务前
以裸 message 帧返回 Propose（带 `negotiationContext`、Negotiation-T metadata，无 taskId/status），引擎按 contextId 与协商 id
关联并触发 `onNegotiation`，续发请求同样不带 taskId；两种载体上的无效协商元数据都显式失败，不会静默当作普通结果。当前实现不使用已废弃的
`startNegotiation` 等状态机接口。
...
- `OrderHttpClientAdapter`：隔离 Order SDK 1.1.19 的版本化类型，业务请求调用公开的 `HttpClient`/`sendSse` API；不直接访问内部
  `HttpSessionService`。1.1.19 的普通 HTTP 桥接路径仍存在已确认的 `ByteBuf` 所有权缺陷，创建客户端时会安装
  `EastcomOrderByteBufWorkaround`；该兼容层只负责释放供应商桥接器已消费但未释放的请求缓冲区，默认 SSE 新路径不经过该桥接器。
- 适配器实例按 `contextId + NE + channel` 隔离并串行访问。1.1.19 JAR 的 `HttpClient` 确实公开静态
...
1.1.19 jar，业务请求仍使用其公开 `HttpClient` API和 TCP/RSocket-RPC 网络栈；执行引擎只在运行时安装下述缓冲区所有权兼容层。模拟平台按
...
SDK 1.1.19 的普通 HTTP 桥接路径在 Windows/JDK 17 下可能按 JVM 默认字符集把 `String` 请求体编码后传入内部 RPC；若服务端一律按 UTF-8 解码，中文
...
失败后应区分“调用方已收到错误”和“平台到 OMC 的连接已释放”。当前 SDK 1.1.19 的普通 HTTP 桥接路径中，
...
### SDK 1.1.19 普通 HTTP 桥接请求缓冲区兼容

`order-shaded-client:1.1.19` 的普通 HTTP `WrapperHttpClient` 路径使用
...
`EastcomOrderByteBufWorkaround` 在供应商 bridge 的出站前一跳安装可共享 handler：先让 bridge 同步复制消息，再在
`finally` 中通过
`ReferenceCountUtil.safeRelease(msg)` 归还原始缓冲区。兼容层有以下边界：

1. 不修改或覆盖原始 1.1.19 jar，也不关闭 Netty 泄漏检测；
...
RSocket-RPC OrderHttpSessionClient；唯一的内部兼容点是上节说明的普通 HTTP 桥接路径 `ByteBuf` 所有权修复：
...
4. .sendSse(SseListener)：使用 1.1.19 默认的 `sendSseNoPort` 长连接实现。SseListener.onHeader(HttpClientResponse)
   收到首帧 HTTP 状态码和响应头；onBodyString(String) 收到字符串分片。不要设置 `useOldSseMethod=true` 回退旧桥接路径。
5. 1.1.19 在 SSE 请求前调用 `loadNeResource` 解析目标；平台必须在 `neParams` 返回 `ip`、`port`、`username`，以及
   `password`（或 `pwd`）。缺少 `ip`/`port` 会导致请求在到达 OMC 前失败。
6. HTTPS 是目标 OMC 路由属性，由平台内部处理。不要在客户端 RSocket 连接上调用 `.secure(...)`，否则会把“平台连接 TLS”和“目标设备
   HTTPS”混为一谈。

补充：1.1.19 JAR 还存在静态 `HttpClient.login(serverInfo, config)`，但原始 v1.8 HTTP
...
7. HttpClient 无持久会话生命周期，无需 logout。底层 Netty 连接池由 SDK 自行管理。
...
6. 发现 completed/failed/canceled/rejected/input-required/auth-required 终态后完成本轮 A2A 解析，并通过 listener 侧异常主动取消
   sendSse 流。现场日志显示转发 SSE 在 A2A 终态后仍保持约 60 秒，但当前可观测边界无法判断最终由 OMC、转发平台、传输通道还是
   SDK 关闭；主动取消可避免等待底层流自然结束，并释放 RSocket requestChannel。若 SDK 吞掉该异常，则退化为旧版等待流关闭的
   行为。`OrderSseTerminalCancelTest` 用「终态后保持流不关」的 OMC stub 验证了组件链路的毫秒级完成；真实平台仍需现场验收。
   供应商 SDK 会先将这个 listener 控制异常打印为 WARN，再包装给适配器；该预期 WARN 带固定标记
   `EXPECTED_A2A_TERMINAL_CANCEL`，并与适配器 `SSE_TERMINAL_CANCELLED` INFO 日志使用同一个 `requestId`。告警可只对该固定标记
   降噪，不应整体屏蔽供应商 logger，以免漏掉真实通道故障。
...
| L-16 | 同一 SDK `HttpClient` 实例的串行复用、关闭及底层连接池生命周期                                                     | 同一 `contextId + NE + channel` 串行调用；`close()` 不调用未公开的 logout；普通 HTTP 桥接路径已安装请求缓冲区释放兼容层                                  | 东信确认线程安全和连接池回收语义并提供正式修复版本；`paranoid` 泄漏检测及长稳压测中线程、连接和直接内存不持续增长 |
...
东信 SDK 的 `SseListener` 会以 INFO 逐帧重复输出单行原始 SSE；将该 logger 设为 WARN 后，协议正文只由
`PROTOCOL` logger 以带关联信息的 pretty 格式输出，不影响东信 SDK 的 WARN/ERROR。
...
| `samples/.../server/EastcomOrderSimulatorServer.java`  | SDK 1.1.19 RSocket-RPC 协议模拟平台          |
| `workflow-engine/.../client/A2ATransport.java`         | 统一传输与订阅资源管理                       |
| `workflow-engine/.../core/WorkflowExecutor.java`       | DAG 遍历引擎，含工作流结构校验               |

## CI 运行环境配置

dev 的示例门禁使用带 `eastcom-sdk` 标签的受控 self-hosted runner。运行器版本须为
v2.327.1 或更新版本，并配置 JDK 17 和 Maven。使用运行器服务账号按本文的依赖安装命令安装东信 jar，
然后在同一账号下执行 `mvn -B clean verify`。服务账号的 Maven 缓存必须包含供应商依赖，
仅安装到开发者登录账号的缓存并不足够。确保示例端口空闲，同一主机上的全量测试应串行运行。

如果 CI 显示“等待 runner 24 小时后取消”，表示该任务尚未执行任何测试。请在实际执行 CI 的仓库
Settings > Actions > Runners 中检查仓库访问权限、Online 状态以及 `self-hosted`、`eastcom-sdk`
两个标签，启动或修复运行器服务后重跑失败任务。PR 不在该运行器执行供应商相关代码；
可信分支推送和手动运行必须通过直连与 Order 两条路径的测试。
配置步骤也见 [贡献指南](../../CONTRIBUTING.md#eastcom-release-gate-dev)。
