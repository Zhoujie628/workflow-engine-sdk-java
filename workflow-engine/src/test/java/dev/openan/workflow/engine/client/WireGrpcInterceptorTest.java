...
class WireGrpcInterceptorTest {
    @Test void capturesRealGrpcMetadataAndProtobufWithoutInventingHttpBody() throws Exception {
        var method = MethodDescriptor.<StringValue, StringValue>newBuilder()
                .setType(MethodDescriptor.MethodType.UNARY).setFullMethodName("test.Protocol/Echo")
                .setRequestMarshaller(ProtoUtils.marshaller(StringValue.getDefaultInstance()))
                .setResponseMarshaller(ProtoUtils.marshaller(StringValue.getDefaultInstance())).build();
        AtomicReference<StringValue> captured = new AtomicReference<>();
        AtomicReference<Metadata> capturedHeaders = new AtomicReference<>();
        var service = ServerServiceDefinition.builder("test.Protocol").addMethod(method,
                ServerCalls.asyncUnaryCall((StringValue request, io.grpc.stub.StreamObserver<StringValue> response) -> {
                    captured.set(request);
                    response.onNext(request);
                    response.onCompleted();
                })).build();
        ServerInterceptor receiver = new ServerInterceptor() {
            public <Q, R> ServerCall.Listener<Q> interceptCall(ServerCall<Q, R> call, Metadata headers,
                    ServerCallHandler<Q, R> next) {
                capturedHeaders.set(headers);
                return next.startCall(new ForwardingServerCall.SimpleForwardingServerCall<Q, R>(call) {
                    public void sendHeaders(Metadata headers) {
                        var key = Metadata.Key.of("x-multi", Metadata.ASCII_STRING_MARSHALLER);
                        headers.put(key, "one"); headers.put(key, "two");
        ...
        Server server = ServerBuilder.forPort(0)
                .addService(ServerInterceptors.intercept(service, receiver)).build().start();
        List<WireLog.Entry> logs = new CopyOnWriteArrayList<>();
        ManagedChannel channel = ManagedChannelBuilder.forAddress("127.0.0.1", server.getPort()).usePlaintext()
                .intercept(new WireGrpcInterceptor(logs::add)).build();
        try {
            Metadata headers = new Metadata();
            var version = Metadata.Key.of("a2a-version", Metadata.ASCII_STRING_MARSHALLER);
            headers.put(version, "1.0");
            headers.put(Metadata.Key.of("authorization", Metadata.ASCII_STRING_MARSHALLER), "Bearer hidden");
            CountDownLatch done = new CountDownLatch(1);
            AtomicReference<Status> status = new AtomicReference<>();
            AtomicReference<StringValue> received = new AtomicReference<>();
            ClientCall<StringValue, StringValue> call = WireLog.call(Map.of("executionId", "exec-1"),
                    () -> channel.newCall(method, CallOptions.DEFAULT.withDeadlineAfter(5, TimeUnit.SECONDS)));
            call.start(new ClientCall.Listener<>() {
                public void onMessage(StringValue value) { received.set(value); }
                public void onClose(Status value, Metadata trailers) { status.set(value); done.countDown(); }
            }, headers);
            call.request(1);
            StringValue payload = StringValue.of("诊断输入");
            call.sendMessage(payload); call.halfClose();
            ...
            var request = logs.stream().filter(e -> e.direction().equals("REQUEST_BODY")).findFirst().orElseThrow();
            assertEquals("protobuf-json-view", request.representation());
            assertEquals("\"诊断输入\"", request.body());
            var response = logs.stream().filter(e -> e.direction().equals("RESPONSE_HEADERS")).findFirst().orElseThrow();
