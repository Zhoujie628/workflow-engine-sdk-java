...
class BusinessBoundaryTest {
    ...
    void identifiersAndWorkflowStateAreIsolatedAcrossExecutions() {
        var workflow =
                Workflow.builder()
                        .name("shared")
                        .steps(List.of(step("one", null, List.of())))
                        .build();
        ...
        assertEquals(
                TaskStatus.PENDING, workflow.getSteps().get(0).getSubtasks().get(0).getStatus());
    ...
    void routingUsesSelectedAncestorsAndCurrentTypedOutputsSeparately() {
        for (List<String> selection :
            ...
            var workflow =
                    Workflow.builder()
                            .name("route-context")
                            .steps(List.of(first, second, route))
                            .build();
                    ...
                    ControlPoint.builder()
                            ...
                            .onRoute(
                                    q -> {
                                        seen.add(q);
                                        return CompletableFuture.completedFuture(
                                                new RouteDecision("endNode", "done"));
                                    })
                            .build();
            assertTrue(
            ...
            int expected =
                    selection == null
                            ? 1
                            : selection.isEmpty() ? 0 : selection.contains("*") ? 2 : 1;
