...
class ObservedHttpClientTest {
    @AfterEach void clearSettings() {
        System.clearProperty("WORKFLOW_ENGINE_PROTOCOL_INCLUDE_BODY");
        System.clearProperty("WORKFLOW_ENGINE_PROTOCOL_MAX_BODY_CHARS");
    }

    @Test void logsActualRequestAndResponseWithoutReencodingOrExposingCredentials() throws Exception {
        ...
        server.createContext("/message", exchange -> {
            capturedBody.add(new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8));
        ...
        try {
            HttpRequest request = HttpRequest.newBuilder(URI.create("http://127.0.0.1:"
                            + server.getAddress().getPort() + "/message"))
                    .header("A2A-Version", "1.0").header("Authorization", "Bearer request-secret")
                    .header("X-Multi", "a").header("X-Multi", "b")
                    .POST(HttpRequest.BodyPublishers.ofString(body)).build();
            HttpResponse<String> result = new ObservedHttpClient(HttpClient.newHttpClient(), records::add)
                    .send(request, HttpResponse.BodyHandlers.ofString());
            assertEquals(response, result.body());
            assertEquals(List.of(body), capturedBody);
            WireLog.Entry sent = records.stream().filter(e -> e.direction().equals("REQUEST_BODY")).findFirst().orElseThrow();
            assertEquals(body.replace("login-secret", "***"), sent.body());
            WireLog.Entry header = records.stream().filter(e -> e.direction().equals("REQUEST_HEADERS")).findFirst().orElseThrow();
            assertEquals(capturedVersion.get(0), header.headers().get("A2A-Version"));
            assertEquals(List.of("a", "b"), header.headers().get("X-Multi"));
            WireLog.Entry received = records.stream().filter(e -> e.direction().equals("RESPONSE_HEADERS")).findFirst().orElseThrow();
    ...
    @Test void streamingFramesKeepUtf8ControlLinesAndArriveBeforeCompletion() {
        List<String[]> frames = new ArrayList<>();
        WireLog.Body collector = new WireLog.Body(true, true, frames::add);
        String first = ": heartbeat\r\nevent: result\r\nid: 7\r\nretry: 100\r\ndata: 中文\r\n\r\n";
        for (byte value : first.getBytes(StandardCharsets.UTF_8)) collector.accept(ByteBuffer.wrap(new byte[]{value}));
    ...
    @Test void oversizedFramesAreExplicitlyDroppedWithoutLeakingPartialSecrets() {
        System.setProperty("WORKFLOW_ENGINE_PROTOCOL_MAX_BODY_CHARS", "256");
        List<String[]> frames = new ArrayList<>();
        WireLog.Body collector = new WireLog.Body(true, true, frames::add);
        collector.accept(ByteBuffer.wrap(("data: {\"password\":\"" + "secret".repeat(100) + "\"}\n\n"
    ...
    @Test void loggingFailureCannotChangeBodyDeliveryAndBodySwitchIsExplicit() throws Exception {
        List<WireLog.Entry> entries = new ArrayList<>();
        System.setProperty("WORKFLOW_ENGINE_PROTOCOL_INCLUDE_BODY", "false");
        WireLog.emit(entries::add, "DIRECT_HTTP", "REQUEST", "1", "/test", "POST",
                null, Map.of(), "serialized-utf8", "private", "", Map.of());
        assertTrue(entries.get(0).visibility().contains("disabled"));
        assertFalse(entries.get(0).body().contains("private"));
        assertDoesNotThrow(() -> WireLog.emit(e -> { throw new IllegalStateException("observer"); },
                "DIRECT_HTTP", "REQUEST", "1", "/test", "POST", null, Map.of(), "serialized-utf8", "", "", Map.of()));
    }
}
