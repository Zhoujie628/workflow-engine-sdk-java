...
final class ObservedHttpClient extends HttpClient {
    ...
    ObservedHttpClient(HttpClient delegate) { this(delegate, null); }
    ...
    private final class Exchange {
        ...
        void emit(String direction, Integer status, Map<String, List<String>> headers,
                  String representation, String body, String detail) {
            WireLog.emit(observer, "DIRECT_HTTP", direction, id, request.uri().toString(), request.method(),
                    status, headers, representation, body,
                    "application-boundary; automatic network headers/TLS/framing=unobserved; " + detail, context);
        }
        void failure(Throwable error) {
            emit("FAILURE", null, Map.of(), "transport-error", "",
                    "errorType=" + error.getClass().getSimpleName());
        }
        HttpRequest observedRequest() {
            return new HttpRequest() {
                @Override public Optional<BodyPublisher> bodyPublisher() {
                    return request.bodyPublisher().map(original -> new BodyPublisher() {
                        ...
                        @Override public void subscribe(Flow.Subscriber<? super ByteBuffer> downstream) {
                            WireLog.Body body = new WireLog.Body(false, true,
                                    value -> emit("REQUEST_BODY", null, Map.of(), "serialized-utf8",
                                            value[0], value[1]));
                            original.subscribe(new Flow.Subscriber<ByteBuffer>() {
                                @Override public void onSubscribe(Flow.Subscription subscription) { downstream.onSubscribe(subscription); }
                                @Override public void onNext(ByteBuffer item) { body.accept(item); downstream.onNext(item); }
                                @Override public void onError(Throwable error) { body.end(true); downstream.onError(error); }
                                @Override public void onComplete() { body.end(false); downstream.onComplete(); }
                            });
                        }
                    });
                }
                @Override public String method() { return request.method(); }
                @Override public Optional<Duration> timeout() { return request.timeout(); }
                @Override public boolean expectContinue() { return request.expectContinue(); }
                @Override public URI uri() { return request.uri(); }
                @Override public Optional<Version> version() { return request.version(); }
        ...
        <T> HttpResponse.BodyHandler<T> handler(HttpResponse.BodyHandler<T> handler) {
            return info -> {
                ...
                WireLog.Body body = new WireLog.Body(sse, type.isEmpty() || type.contains("json")
                        || type.startsWith("text/") || type.contains("xml"),
                        value -> emit("RESPONSE_BODY", info.statusCode(), Map.of(),
                                sse ? "raw-sse-frame" : "raw-body", value[0], value[1]));
                HttpResponse.BodySubscriber<T> target = handler.apply(info);
                return new HttpResponse.BodySubscriber<T>() {
                    @Override public CompletionStage<T> getBody() { return target.getBody(); }
                    @Override public void onSubscribe(Flow.Subscription subscription) {
                        target.onSubscribe(new Flow.Subscription() {
                            @Override public void request(long count) { subscription.request(count); }
                            @Override public void cancel() { body.end(true); subscription.cancel(); }
                        });
                    }
                    ...
                    @Override public void onError(Throwable error) { body.end(true); failure(error); target.onError(error); }
                    @Override public void onComplete() { body.end(false); target.onComplete(); }
                };
            };
        }
    }
    ...
    @Override public <T> HttpResponse<T> send(HttpRequest request, HttpResponse.BodyHandler<T> handler)
        ...
        try { return delegate.send(exchange.observedRequest(), exchange.handler(handler)); }
        catch (IOException | InterruptedException | RuntimeException error) { exchange.failure(error); throw error; }
    }
    ...
    @Override public <T> CompletableFuture<HttpResponse<T>> sendAsync(
        ...
        try {
            ...
            future.whenComplete((result, error) -> { if (error != null) exchange.failure(error); });
            return future;
        } catch (RuntimeException error) { exchange.failure(error); throw error; }
    }
    ...
    @Override public <T> CompletableFuture<HttpResponse<T>> sendAsync(
        ...
        Exchange exchange = new Exchange(request);
        try {
            CompletableFuture<HttpResponse<T>> future = delegate.sendAsync(exchange.observedRequest(),
                    exchange.handler(handler), pushHandler);
            future.whenComplete((result, error) -> { if (error != null) exchange.failure(error); });
            return future;
        } catch (RuntimeException error) { exchange.failure(error); throw error; }
    }
