...
public final class WireLog {
    ...
    private static final Pattern SECRET_FORM = Pattern.compile(
    ...
    public record Entry(String boundary, String direction, String requestId, String target, String method,
                        Integer status, Map<String, List<String>> headers, String representation,
                        String body, String visibility, Map<String, String> correlation) {}

    public static Map<String, String> context() { return CONTEXT.get(); }
    ...
    public static <T> T call(Map<String, String> context, java.util.function.Supplier<T> action) {
        ...
        try { return action.get(); } finally { CONTEXT.set(previous); }
    }
    ...
    public static void inContext(Map<String, String> context, Runnable action) {
        Map<String, String> previous = CONTEXT.get();
        CONTEXT.set(Map.copyOf(context));
        try { action.run(); } finally { CONTEXT.set(previous); }
    }
    ...
    static int limit() {
        String value = System.getProperty("WORKFLOW_ENGINE_PROTOCOL_MAX_BODY_CHARS",
                System.getenv("WORKFLOW_ENGINE_PROTOCOL_MAX_BODY_CHARS"));
        try { return Math.max(256, Integer.parseInt(value)); }
        catch (RuntimeException ignored) { return 100_000; }
    }
    ...
    public static String redact(String text) {
        ...
        for (int cursor = 0; cursor < text.length(); cursor++) {
            ...
            String key;
            try { key = JSON.readValue(text.substring(cursor, endOfString), String.class); }
    ...
    static boolean sensitive(String name) {
        ...
        return key.contains("authorization") || key.contains("cookie") || key.contains("token")
                ...
                || key.equals("x-api-key") || key.equals("api-key") || key.equals("apikey");
    }
    ...
    public static Map<String, List<String>> safeHeaders(Map<String, List<String>> headers) {
        ...
        headers.forEach((key, values) -> result.put(key,
                sensitive(key) ? List.of("***") : List.copyOf(values)));
        return Collections.unmodifiableMap(result);
    }

    /** Actual serialized data only. SDK-decoded data must explicitly say decoded-data. */
    public static void record(String boundary, String direction, String requestId, String target,
                              String method, Integer status, Map<String, List<String>> headers,
                              String representation, String body, String visibility) {
        emit(null, boundary, direction, requestId, target, method, status, headers,
                representation, body, visibility, context());
    }

    static void emit(Consumer<Entry> observer, String boundary, String direction, String requestId,
                     String target, String method, Integer status, Map<String, List<String>> headers,
                     String representation, String body, String visibility, Map<String, String> context) {
        try {
            ...
            Entry entry = new Entry(boundary, direction, requestId, redact(target), method, status,
                    safeHeaders(headers), representation, safeBody, visibility, Map.copyOf(context));
            if (observer != null) observer.accept(entry);
            else if (LOG.isDebugEnabled()) LOG.debug("{}", WireLogFormatter.format(entry,
                    setting("WORKFLOW_ENGINE_PROTOCOL_PRETTY", true)));
    ...
    public static final class Body {
        ...
        public synchronized void end(boolean interrupted) {
            if (ended) return;
            ended = true;
            if (pendingCr) { lineEnd(); pendingCr = false; }
        ...
        private void flush(boolean interrupted) {
            String content = dropped ? "(body/frame dropped: capacity exceeded)"
                    : !textual ? "(binary body omitted)"
                    : new String(bytes.toByteArray(), StandardCharsets.UTF_8);
            // Decode only complete frames/body, keeping UTF-8 code points intact across chunks.
            String visible = "observedBytes=" + size + (dropped ? "; body=dropped-capacity" : "")
                    + (interrupted ? "; stream=interrupted" : "");
            try { sink.accept(new String[]{content, visible}); }
            catch (RuntimeException ignored) { LOG.warn("Protocol observation failed; record dropped"); }
            bytes.reset(); size = 0; dropped = false;
        }
    }
}
